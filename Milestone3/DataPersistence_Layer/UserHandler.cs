    ...
    class UserHandler
    {
        ...
        public bool SaveNewUser (String email, String pass)
        {
            ...
            try
            {
                ...
                command.CommandText =
                            "INSERT INTO Users (Email_UserID, Password)" +
                                "VALUES (@email, @pass)";

                SQLiteParameter email_param = new SQLiteParameter(@"email", email);
                SQLiteParameter pass_param = new SQLiteParameter(@"pass", pass);
        ...
        public void SaveUser (DummyUser dummyUser)
        {
            if (DummyUser.hashDummyUsers.Contains(dummyUser.getEmail()))
                ChangeDummyUserPass(dummyUser.getEmail(), "", dummyUser.getPass());
            else
                SaveNewUser(dummyUser.getEmail(), dummyUser.getPass());
        }
        ...
        public static List<String> getBoardIDsOfUser(SQLiteConnection connection, string email)
        {
            ...
            try
            {
                ...
                while (dataReader.Read())
                {
                    string boardID = (string)dataReader["BoardID"];
                    currEmailBoardIDs.Add(boardID);
                }
            }
            catch (Exception e)
            {
