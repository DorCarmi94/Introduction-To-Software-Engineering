...
namespace Milestone3.Bussiness_Layer.Interface
{
    public class IColumn
    {
        ...
        public String checkIfCanCreateNewTask(String columnID, String UserID, String Description, String taskTitle, int day, int month, int year)
        {
            if (!(Guid.TryParse(columnID, out Guid ColumnID)))
            {
                log.Error("Error trying to parse column ID entered");
                return "Error trying to parse column ID entered";
            }
            else
            {
                Column theCol = myColumns.getByColumnID(ColumnID);
                if(!Task.tryCheckTitle(taskTitle))
                {
                    return "Couldn't create new task. Problem with task's title.\n" +
                        "Must conatian a least one letter and be up to "+Task.maxCharsTitle+" characters at the most";
                }
                if (!Task.tryCheckDescription(Description))
                {
                    return "Couldn't create new task. Problem with task's description.\n" +
                        "Can be up to " + Task.maxCharsDesc + " characters at the most";
                }
                if (Task.tryCheckDate(year,month,day).Equals(Task.ERROR_DATE))
                {
                    return "Couldn't create new task. Problem with task's due date.\n" +
                        "Can't create a task with due date in the past";
                }
                if(theCol.getColumnLimit()!=-1 && theCol.getNumOfTasks()+1>theCol.getColumnLimit())
                {
                    return "Couldn't create new task. Problem with cloumns task limit.\n" +
                        "The current task limit in the column "+theCol.getColName()+" is: "+theCol.getColumnLimit();
                }
                else
                {
...

        {
            if (!(Guid.TryParse(taskID, out Guid TaskID_guid)) & !(Guid.TryParse(colID, out Guid colID_guid)))
            {
                ...
                return "Error trying to parse column ID or task ID that was entered";
            }
            else
            {
